...
from Cluster.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, PM_TTI_SHOW_OFF, IMDB, \
    SINGLE_BUTTON, PM_SPELL_CHECK_REPLY, IMDB_TEMPLATE
...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("🔙 𝙱𝙰𝙲𝙺", callback_data=f"pmnext_{req}_{key}_{off_set}"),
        ...
        btn.append(
            [InlineKeyboardButton(f"📄 {round(int(offset) / 10) + 1} / {round(total / 10)}", callback_data="pages"),
             InlineKeyboardButton("𝙽𝙴𝚇𝚃 ➡️", callback_data=f"pmnext_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
...
async def auto_filter(client, msg, spoll=False):
    ...
    pre = 'filep' if settings['file_secure'] else 'pmfile'
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"📄 1/{round(int(total_results) / 10)}", callback_data="pages"),
             InlineKeyboardButton(text="𝙽𝙴𝚇𝚃 ➡️", callback_data=f"pmnext_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
